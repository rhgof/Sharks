...
Test files use `source(codeFile(...))` to load the real source files, with a `setwd` guard for testthat compatibility. Current test coverage: `cropToArea()`, `imosFiles()`, `readDrumlineReports()`.
...
- **CRAN packages**: tidyverse, terra, tidyterra, ncdf4, patchwork, gifski, paletteer, viridis, scales, readxl, pdftools
...
Shark DB (CSV) + IMOS Satellite Portal (NetCDF) + Shapefiles
        ↓
  Read/Parse Layer  →  readSharkDB.R, readIMOSFileListing.R (imosFiles), readChlAFiles.R, readDrumlineReports.R
        ↓
...
- **PDF extraction**: `readDrumlineReports.R` uses `pdftools::pdf_data()` (word-level x,y coordinates) rather than `pdf_text()` (plain text). This avoids the space-splitting ambiguity that caused species truncation and column misalignment in the previous Python approach.
...
- `Inputs/2023-24-NSW-SMART-drumline-annual-report.pdf`: NSW SMART drumline catch data (Appendix 2), parsed by `readDrumlineReports.R`
- `Inputs/Appendix_2_cleaned.csv`: Extracted drumline catch data (generated by `readDrumlineReports()`)
